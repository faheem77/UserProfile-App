...
def user_signup(request):
	if request.method == 'POST':
		...
		try:
			...
			return render(request, 'user_profile/user_signup.html')
		return render( request, ' user_profile/user_signup.html')
...
def user_login(request):
	if request.method == 'POST':
		...
		userpass = request.POST['password']
		try:
			user_obj = authenticate(userpass= userpass, password= userpass)
...
def user_profile(request, user_id):
	if request.method == 'POST':
		...
		user__profile_obj = UserProfile.objects.get(id= user_id)
		...
		if fs_handle.exists(img_name):
			fs_handle.delete(img_name= img_name)
		fs_handle.save(img_name, user_img)
		user_profile.user_obj.profile_img= img_name
		user__profile_obj.save()
		user__profile_obj.refresh_from_db()
		return render(request, 'user_profile/my_profile.html', {'my_profile': user__profile_obj})
	if (request.user.is_authenticated and request.user.id == user_id):
		user_obj = User.objects.get(id= user_id)
		return render( request, ' user_profile/my_profile.html', {'ny_profile': user_profile})